...
let lastSMA = null, lastAbove = null;
...
async function loadData(){
  ...
  smaSeries   .setData(calcSMA(candles,14));
  toggleType();
...
function connectWS(pair,inter){
  ...
  ws.onmessage=e=>{
    ...
    if(!k.x) return; // aguardamos o candle fechar
...
function calcNextSMA(close){
  ...
  const avg=smaQueue.reduce((s,x)=>s+x,0)/smaQueue.length;
  return avg;
}
